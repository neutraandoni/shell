...
sys.stdout.write("\x1b]2;V O I D |CORT| D O W N E D\x07")
def modifications():
	print ("MANT = ss")
...
info = """
[\033[91mSIN\033[00m] \033[91mSMACK was made by souls
it is the best perl script to ddos, downing a home in 0.5 seconds
"""
...
def main():
	...
	while True:
		sys.stdout.write("\x1b]S M A C K\x07")
		sin = input("\033[1;00m[\033[91mSmack\033[1;00m]-\033[91m家\033[00m ").lower()
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("clear")
			print (altbanner)
			...
			sfound = 0
			sys.stdout.write("\x1b]2;S M A C K |{}| F O U N D\x07".format (sfound))
			try:
				host = sin.split(" ")[1]
			...
			try:
				...
				while True:
					try:
						...
						sys.stdout.write("\x1b]2;S M A C K |{}ms| D E M O N S\x07".format (stop))
						print ("Sinfull: {}:{} | Time: {}ms [\033[91mUP\033[00m]".format(ip, port, stop))
						sock.close()
						time.sleep(1)
					except socket.error:
						sys.stdout.write("\x1b]2;S M A C K |TIME OUT| D E M O N S\x07")
